...
second(Lengths) ->
  Sparse = asList(round(64, {0, values(255)}, {0, lists:flatten(Lengths, [17, 31, 73, 47, 23])})),
  hexit(xorbit(Sparse, [])).

round(0, {_, Values}, {_, _}) -> Values;
round(Round, {Cursor, Values}, {Skip, Lengths}) ->
  {C2, S2, V2} = knit({Cursor, Values}, {Skip, Lengths}),
  round(Round - 1, {C2, V2}, {S2, Lengths}).

first(Max, Lengths) ->
  {_, _, Result} = knit({0, values(Max)}, {0, Lengths}),
  maps:get(0, Result) * maps:get(1, Result).

asList(Map) -> [ maps:get(Key, Map) || Key <- lists:sort(maps:keys(Map))].

values(Max) -> maps:from_list([ {V, V} || V <- lists:seq(0, Max)]).

knit({Cursor, Values}, {Skip, []}) -> {Cursor, Skip, Values};
knit({Cursor, Values}, {Skip, [Length|T]}) ->
  Selection = selection(Values, Cursor, Length),
  Reversed = reverse(Values, Selection),
  knit({key(Cursor + Length + Skip, Values), Reversed}, {Skip + 1, T}).

reverse(Values, Keys) ->
  maps:merge(Values, maps:from_list(lists:zipwith(fun(A, B) -> {A, B} end, Keys, reverse(Values, Keys, [])))).

reverse(_, [], Selection) -> Selection;
reverse(Values, [Key|T], Selection) -> reverse(Values, T, [maps:get(Key, Values)|Selection]).

selection(Values, Cursor, Count) -> selection(Values, Cursor, Count, []).
selection(_, _, 0, Keys) -> Keys;
selection(Values, Cursor, Count, Keys) -> selection(Values, Cursor + 1, Count - 1, [key(Cursor, Values)|Keys]).

key(Cursor, Values) -> mod(Cursor, maps:size(Values)).

xorbit([], Result) -> lists:reverse(Result);
xorbit(Sparse, Result) ->
  {H, T} = lists:split(16, Sparse),
  xorbit(T, [lists:foldr(fun(N, A) -> A bxor N end, 0, H)|Result]).

hexit(Input) -> lists:flatten([ [hex(N div 16), hex(N rem 16)] || N <- Input]).
...
case_part1_a_test() -> 12 = first(4, [3,4,1,5]).
case_part1_b_test() -> 826 = first(255, [120,93,0,90,5,80,129,74,1,165,204,255,254,2,50,113]).
...
% For example, if the previous example was your first round, you would start your second round with the same length sequence
...
case_part2_hex_test() -> "4007ff" = hexit([64, 7, 255]).
case_part2_xor_test() -> 64 = hd(xorbit([65,27,9,1,4,3,40,50,91,7,6,0,2,5,68,22], [])).
case_part2_a_test() -> "a2582a3a0e66e6e86e3812dcb672a272" = second("").
case_part2_b_test() -> "33efeb34ea91902bb2f59c9920caa6cd" = second("AoC 2017").
case_part2_c_test() -> "3efbe78a8d82f29979031a4aa0b16a9d" = second("1,2,3").
case_part2_d_test() -> "63960835bcdc130f0b66d7ff4f6a5a8e" = second("1,2,4").
case_part2_e_test() -> "d067d3f14d07e09c2e7308c3926605c4" = second("120,93,0,90,5,80,129,74,1,165,204,255,254,2,50,113").